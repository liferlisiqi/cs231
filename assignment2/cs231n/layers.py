...
def relu_backward(dout, cache):
    ...
    dh = np.zeros(x.shape)
    dh[x >= 0] = 1
    dx = dh * dout # * is element multiply element
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        mu = x.mean(axis=0)
        # step 2: subtract mean vector of every training samples
        xmu = x - mu
        ...
        # step 6: invert the sqrtvar
        ivar = 1. / sqrtvar
        # step 7: execute the normalization
        xhat = xmu * ivar
        # step 8 and step 9: normalize the two transformmation steps
        gammax = gamma * xhat
        out = gammax + beta
        
        cache = (gamma, xhat, ivar, xmu, sqrtvar, var, eps)
...
def batchnorm_backward(dout, cache):
    ...
    N, D = dout.shape
    gamma, xhat, ivar, xmu, sqrtvar, var, eps = cache
    ###########################################################################
    ...
    # step 4:
    dsq = np.ones((N, D)) * dvar / N
    # step 3:
    dxmu2 = 2 * xmu * dsq
    # step 2:
    dx1 = dxmu1 + dxmu2
    dmu = -1 * dx1.sum(axis=0)
    # step 1:
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(Hp):
        for j in range(Wp):
            x_sub = x[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
            x_mask = np.zeros_like(x_sub)
            x_mask[x_sub == np.max(x_sub, axis=(2,3), keepdims=True)] = 1
            dx[:,:,i*stride:i*stride+HH,j*stride:j*stride+WW] +=x_mask* (dout[:,:,i,j])[:,:,None,None]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
